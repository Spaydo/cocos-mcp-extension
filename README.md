...
# 預期回應: {"status":"ok","tools":88,...}
```

## 支持的工具

共 88 個工具，分為 11 個類別（7 個核心 + 4 個進階）。MCP 工具名稱格式為 `類別_動作`。
...
| 工具名稱 | 說明 |
|----------|------|
| `prefab_query` | 查詢 prefab 內部節點階層結構（不修改場景，需有場景開啟） |
...
| 工具名稱 | 說明 |
|----------|------|
| `debug_get_logs` | 取得最近的控制台/編輯器日誌（支援 errors_only、since 時間戳過濾） |
